<template>
  <form @submit.prevent="submit()" class="form-inline">
    ...
    <div class="row">
      <div class="col-md-4">
        <input
          v-model="params.customer_address_city"
      ...
      <div class="col-md-4">
        <input
          v-model="params.customer_address_street"
      ...
      <div class="col-md-4">
        <div class="row">
          <div class="col-6">
            <input
              v-model="params.customer_address_house"
              name="customer_address_house"
              type="text"
              class="form-input sm"
              placeholder="Дом"
              required
          ...
          <div class="col-6">
            <input
              v-model="params.customer_address_flat"
              name="customer_address_flat"
              type="text"
              class="form-input sm"
    ...
    <div class="form-disclaimer">
      <p>
        Факт отправки формы подтверждает ваше согласие на
        <a
          href="https://www.barrier.ru/policy/"
          target="_blank"
        >передачу и обработку персональных данных</a>, а также получение информационных рассылок
      </p>
    </div>
  ...
  methods: {
    submit() {
      ...
      this.required.forEach(function(key) {
        if (
          key === "customer_email" &&
          !/[a-zA-Z0-9-._]+@[a-zA-Z0-9-._]+\.[a-z]+/i.test(self.params[key])
        ) {
          self.errors.push("Введите корректный email");
        } else if (
          key === "customer_phone" &&
          !/\+7 \([0-9]{3}\) [0-9]{3}-[0-9]{2}-[0-9]{2}/.test(self.params[key])
        ) {
          self.errors.push(
      ...
      if (this.errors.length === 0) {
        axios
          .post(
            this.$store.getters.apiUrl + "order/create",
            Object.assign({}, this.params, this.$store.getters.cartForOrder)
          )
          .then(function() {
